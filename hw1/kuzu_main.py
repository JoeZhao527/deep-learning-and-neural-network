...
def main():
    ...
    if list(net.parameters()):
        ...
        # this loop pass the dataset to the network for [args.epochs] times
        for epoch in range(1, args.epochs + 1):
            # args is not used in neither train nor test
            # net is the newwork structure that we need to implenment (TODO)
            # train_loader contains the the whole dataset
            # epoch contains how many times has the dataset has been passed into the network
            train(args, net, device, train_loader, optimizer, epoch)

            # from every epoch we want to test how does the network works
